                        ...
                        <div class="contentarea">                        	
                            <div class="row">
                            	<div class="col-sm-12 module_cont pb20 animate" data-anim-type="fadeInLeft" data-anim-delay="300">
                                    <h3>${complaint.getTitle()}<small>${complaint.getId()}</small></h3>
                                    <table class="table table-hover">
                                        ...
                                        <tbody>
                                        <tr>
                                            <td>${complaint_user.getName()}</td>
                                            <td>${complaint_user.getId()}</td>
                                            <td>
                                    ...
                                    <div class="container">
                                        <c:forEach items="${pictures}" var="pic" varStatus="i">
                                            <c:if test="${(i.index+1)%3 ==0}">
                                                <div class="row">
                                            </c:if>
                                            <c:if test="${pic.getType() == 1}">
                                                <video width="320" height="240" controls>
                                                    <source src="<c:url value="/image/"/>${pic.getFilename()} " type="video/mp4">
                                                    您的浏览器不支持Video标签。
                                                </video>
                                            </c:if>
                                            <c:if test="${pic.getType() == 0}">
                                                <div class="col-sm-4">
                                                    <img src="<c:url value="/image/"/>${pic.getFilename()} ">
                                                </div>
                                            </c:if>
                                            <c:if test="${(i.index+1)%3 ==0}">
                                                </div>
                                ...
                                <hr size="2"/>
                                    <h3>新建/修改回复</h3>
                                    <form action="<c:url value="/complaint/reply.do?complaint_id="/>${complaint.getId()}"  method="post" role="form">
                                        <textarea name="reply">${complaint.getReply()}</textarea><br/>
                                        <input type="hidden" name="page">
                                        <input type="submit" value="回复" style="width: 200px;">
                                    </form>
                                    <br/>
                                    <hr size="2"/>
                                <h3>用户评价:</h3>
                                <p>${complaint.getJudge()}
                                </p>
                            </div>
                            </div>
                        </div>
